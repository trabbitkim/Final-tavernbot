...
function keyboard(env) {
  // Telegram rejects the ENTIRE message if a button URL is missing or
  // malformed. So only attach the button when the link actually looks real.
...
async function handleUpdate(update, env) {
  ...
  if (joined) {
    console.log("join via service message:", joined.map((m) => m.first_name).join(", "));
